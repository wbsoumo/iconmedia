...
<head>
    ...
    <title>SubID Performance Report · Admin Control Panel</title>
...
<body>
    <div class="header">
        <h1><i class="fas fa-tags" style="color: #3b82f6; margin-right: 8px;"></i> SubID Performance Report (Sub1)</h1>
    ...
    <div class="card table-responsive">
        <table>
            ...
            <tbody>
                <?php if (empty($rows)): ?>
                    <tr>
                        <td colspan="5" style="text-align: center; color: #64748b; padding: 24px;">No SubID tracking data found for the selected date range.</td>
                    </tr>
                <?php else: ?>
                    ...
                    ?>
                        <tr>
                            <td style="font-weight: 600; color: #fff;"><?= htmlspecialchars($r['sub1']) ?></td>
                            <td><span class="badge"><?= (int)$r['clicks'] ?></span></td>
                            <td><?= (int)$r['conversions'] ?></td>
                            <td style="color: #4ade80;">$<?= number_format((float)$r['payout'], 2) ?></td>
                            <td style="color: #60a5fa; font-weight: 600;"><?= number_format($cr, 2) ?>%</td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
